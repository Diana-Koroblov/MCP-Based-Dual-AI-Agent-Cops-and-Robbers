...
> **Detailed Appendix Note:** The original PDF contains a highly detailed appendix (Chapter 20) that breaks down all the submission guidelines step-by-step. This MD file condenses that section into a much shorter and concise checklist.

---

### Important Missing Project-Relevant Information (To add to the MD)

Here are the crucial technical configurations, code examples, and explicit rules from the original PDF that should be included in your project guidelines:

#### A. Mandatory Configuration Examples

**Ruff Linter Configuration (`pyproject.toml`)**
Zero Ruff violations are allowed. Your configuration must explicitly include the following rules:
...
**Rate Limit Configuration Format (`config/rate_limits.json`)**
Rate limits must be enforced by the API Gatekeeper and loaded from a versioned configuration file:
...
#### B. Strict "Do's and Don'ts" Tables

**Table: Hardcoded Values vs. Configuration** 

| Category | Incorrect Usage (Forbidden) | Correct Usage |
| --- | --- | --- |
...
**Table: Package Management (`uv` vs. `pip`)** 

| Task | Correct (`uv` - Mandatory) | Forbidden |
| --- | --- | --- |
| Install dependencies | `uv sync` | `pip install` |
| Add dependency | `uv add <pkg>` | `pip install <pkg>` |
| Run script / tests | `uv run python script.py` <br>

<br> `uv run pytest tests/` | `python script.py` <br>

<br> `python -m pytest` |
| Lock dependencies | `uv lock` | `pip freeze` |

#### C. Expanded Documentation Requirements (From Appendix 20)

To achieve the highest level of excellence, the project documentation must contain specific elements:

* 
**In the PRD (`docs/PRD.md`):** Must include measurable KPIs, acceptance criteria, market analysis, target audience identification, and a detailed timeline with milestones.


* 
**In the Architecture Plan (`docs/PLAN.md`):** Must utilize the C4 Model (Context, Container, Component, Code), UML diagrams for complex processes, deployment diagrams, and formal Architectural Decision Records (ADRs) with rationale and trade-offs.


* **In Research & Analysis Visualizations:** High-quality graphs are required. Specifically expected are Box plots, Heatmaps, Scatter plots, Line charts, and Bar charts. Recommended tools for these visualizations include Matplotlib, Seaborn, Plotly, Tableau, or D3.js.


1. Guidelines for Managing TODO.md
According to the project requirements, the TODO.md file must serve as a comprehensive project management tool rather than a simple checklist. It must be divided into clear phases and milestones, detailing priorities, statuses, and assigned responsibilities. Crucially, every single task must have a specific "Definition of Done" (DoD).

Recommended Template for TODO.md:

Markdown
# Project Tasks and Tracking (TODO.md)

## Phase 1: Planning and Documentation (Milestone 1)
| ID | Task Description | Priority | Status | Assignee | Definition of Done (DoD) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1.1 | Write main PRD document | High | Completed | [Name] | Document approved, includes KPIs and target audience. |
| 1.2 | Write PLAN architecture document | High | In Progress | [Name] | C4 Model diagram ready, all APIs documented. |
| 1.3 | Create PRD for Algorithm X | Medium | Not Started | [Name] | Document defines input/output and performance metrics. |

## Phase 2: Infrastructure and SDK (Milestone 2)
| ID | Task Description | Priority | Status | Assignee | Definition of Done (DoD) |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 2.1 | Setup workspace with `uv` | High | Not Started | [Name] | `uv.lock` created, Ruff rules configured. |
| 2.2 | Develop base SDK layer | High | Not Started | [Name] | Class implemented, 85% test coverage achieved. |
2. Enforcing the 150-Line Limit
No single code file may exceed 150 lines of code. Note: Blank lines and comment lines (including docstrings) do not count toward this limit. However, you must never compress or cram code (e.g., writing multiple statements on a single line) just to bypass the restriction.

To stay under the limit, utilize the following required splitting strategies:

Extract Constants: Move all configuration variables, static messages, and default values to a dedicated constants.py file.

Extract Models: Separate data structures and classes into their own dedicated model files.

Extract Helpers: Move independent, reusable logic (like data validation or string processing) into separate helper functions and files.

50/50 Split & Mixins: If a class handles multiple responsibilities (e.g., reading and writing logic), split it into two logical halves. Use small, independent Mixin classes where each Mixin handles exactly one concern.

💡 Automation Tip: Since the guidelines require automated checking for file size, it is highly recommended to write a custom test in your tests/ directory. This test should scan the src/ directory, count the active lines of code (ignoring comments/blanks), and automatically fail the pytest run if any file exceeds the 150-line threshold.

### 3. Required Testing Workflow and Automation
While continuous manual testing is not explicitly mandated minute-by-minute, the combination of required methodologies dictates that tests must be run frequently and automatically, strictly using uv:

Exclusive Use of uv for All Execution: All test executions must be performed exclusively using uv. Direct calls to pytest, python -m pytest, or pip are strictly forbidden across the codebase, scripts, documentation, and CI/CD pipelines. The only acceptable command to run tests is uv run pytest tests/.

CI/CD Pipeline Automation: The project must implement an automated CI/CD pipeline. Tests and coverage reports must be executed automatically upon code submission. Even within the automated CI/CD environments, all tools and test executions must strictly utilize uv run.

TDD and Frequent Local Execution: Development must strictly follow the Test-Driven Development (TDD) cycle: Red, Green, Refactor. Because tests must be written before or alongside the implementation, developers are implicitly required to execute the uv test command frequently during the coding process to ensure continuous compliance with the mandatory 85% minimum test coverage threshold.